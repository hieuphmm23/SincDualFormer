...
def _parse_str_list_env(name: str, default):
    raw = os.environ.get(name)
    if raw is None or str(raw).strip() == "":
        return list(default)
...
def _norm_ablation_mode(mode: str) -> str:
    return str(mode).strip().upper().replace("-", "_").replace(" ", "_")
...
ROOT_GDF = os.environ.get("ROOT_GDF_2B", "bci/BCICIV_2b_gdf")
ROOT_LABELS = os.environ.get("ROOT_LABELS_2B", "bci/true_labels_2b")
AUG_MODE = normalize_aug_mode(os.environ.get("AUG_MODE", "SR_BANDMIX"))
OUTPUT_ROOT = os.environ.get("OUTPUT_ROOT", os.path.join("results", "2b"))
...
FLATTEN_SIZE = 320
KERNEL_SHORT = 64
KERNEL_LONG = 96  # Variant A: shorter Mu temporal kernel
DIL_SHORT = 1
...
# The reported IV-2b setting uses the default value below.
...
# False = apply AdamW weight decay to every parameter.
...
SAMPLE_RATE = 250.0
DATASET_TYPE = "B"  # IV-2b: 2 classes, 3 EEG channels
...
# ============== Architecture Ablation Config ==============
# Default: run the full model plus all requested ablations in one execution.
# You can override from shell, for example:
#   ABLATION_MODES="FULL,WO_SINCNET" python this_file.py
ABLATION_MODES = [
    _norm_ablation_mode(x) for x in _parse_str_list_env(
        "ABLATION_MODES",
        [
            "FULL",
            "WO_BRANCH_I",
...
VALID_ABLATION_MODES = {
    "FULL",
    "WO_BRANCH_I",                 # remove Branch I = Mu / long temporal branch
    "WO_BRANCH_II",                # remove Branch II = Beta / short temporal branch
    "WO_TRANSFORMER_ENCODER",      # remove Transformer encoder; keep CNN tokens + PE
    "WO_SINCNET",                  # remove SincNet path inside both branches
    "WO_TEMPORAL_CONV_LONG_SHORT", # remove learned temporal Conv path in both long/short branches
    "WO_INCEPTION_TCN",            # replace Inception-TCN with identity
    "WO_POINTWISE_CONV_FUSION",    # replace learnable 1x1 fusion by fixed average/sum
    "WO_BANDSE",                   # remove BandSE recalibration in both Conv/Sinc paths
}
for _m in ABLATION_MODES:
    if _m not in VALID_ABLATION_MODES:
        raise ValueError(f"Unknown ABLATION_MODE={_m}. Valid modes: {sorted(VALID_ABLATION_MODES)}")

# Single-subject main() uses this mode. The multi-subject runner overrides per ablation.
ABLATION_MODE = _norm_ablation_mode(os.environ.get("ABLATION_MODE", ABLATION_MODES[0]))
if ABLATION_MODE not in VALID_ABLATION_MODES:
    raise ValueError(f"Unknown ABLATION_MODE={ABLATION_MODE}. Valid modes: {sorted(VALID_ABLATION_MODES)}")
...
# ============== BCI Competition IV-2b loader ==============
def _pick_3_eeg_indices_2b(raw):
    """Pick the three EEG channels used in BCI Competition IV-2b: C3, Cz, C4."""
    names = raw.info["ch_names"]
    target_order = ["c3", "cz", "c4"]
    picks = []
    for target in target_order:
        hit = None
        for i, name in enumerate(names):
            nn = _norm_ch_name(name)
            if nn == target or nn.endswith(target):
                hit = i
                break
        if hit is not None:
            picks.append(hit)
    ...
    if len(picks) != 3:
        ...
        if len(eeg_picks) >= 3:
            ...
            non_eog_by_name = [i for i, n in enumerate(names) if "eog" not in str(n).lower()]
            picks = non_eog_by_name[:3]

    if len(picks) != 3:
        raise RuntimeError(f"[IV-2b] expected 3 EEG channels, got {len(picks)} from {names}")
    picked_names = [names[i] for i in picks]
    return picks, picked_names


def _read_raw_and_events_2b(gdf_path: str):
    """Read one IV-2b GDF file and return raw, events, event_id, and C3/Cz/C4 picks."""
    raw = mne.io.read_raw_gdf(gdf_path, preload=True, verbose=False)
    events, event_id = mne.events_from_annotations(raw, verbose=False)
    picks, picked_names = _pick_3_eeg_indices_2b(raw)
    return raw, events, event_id, picks, picked_names


def _find_train_event_codes_2b(event_id: dict):
    """Find IV-2b class cue codes in label order 1..2: 769=left, 770=right."""
    left_id = _lookup_event_id(event_id, ["769", "t1", "left", "left hand", "class 1", "cue onset left"])
    right_id = _lookup_event_id(event_id, ["770", "t2", "right", "right hand", "class 2", "cue onset right"])
    return left_id, right_id


def _find_eval_event_code_2b(event_id: dict):
    """Find unknown/evaluation event code for IV-2b evaluation files."""
    return _lookup_event_id(event_id, ["783", "t0", "unknown", "cue unknown", "start of trial"])


def _derive_labels_from_event_codes_2b(sel_events, class_ids):
    """Derive 1..2 labels from IV-2b class cue events."""
    codes = sel_events[:, 2]
    y = np.zeros_like(codes, dtype=np.int64)
    for label, event_code in enumerate(class_ids, start=1):
        if event_code is not None:
    ...
    if np.any(y == 0) and sel_events.shape[0] > 0:
        bad = np.unique(codes[y == 0]).tolist()
        raise RuntimeError(f"[IV-2b] Some trials are not class events: {bad}")
    return y


def _resolve_gdf_2b(root_gdf: str, subject: int, session: int, phase: str) -> str:
    """Find B0101T/B0104E style IV-2b GDF files robustly."""
    stem = f"B{subject:02d}{session:02d}{phase}"
    patterns = [
        f"{stem}.gdf", f"{stem}.GDF",
        f"*{stem}.gdf", f"*{stem}.GDF",
        f"**/{stem}.gdf", f"**/{stem}.GDF",
        f"**/*{stem}.gdf", f"**/*{stem}.GDF",
    ]
    hits = []
    for pat in patterns:
        hits.extend(glob.glob(os.path.join(root_gdf, pat), recursive=True))
    hits = sorted(set(hits), key=lambda p: (len(os.path.basename(p)), p))
    if not hits:
        raise FileNotFoundError(f"Missing IV-2b GDF for {stem} under {root_gdf}")
    return hits[0]


def _resolve_label_mat_2b(root_labels: str, subject: int, session: Optional[int] = None, phase: str = "E") -> Optional[str]:
    """
    Resolve IV-2b label .mat safely.

    Critical fix:
    - B0504E.gdf must map to B0504E.mat.
    - B0505E.gdf must map to B0505E.mat.
    - An evaluation session must never fall back to B0501T/B0502T/B0503T.

    The previous resolver used broad subject patterns such as *B05*.mat.
    Because B0501T.mat is alphabetically/length-wise a valid broad hit, it could
    be selected for B0504E.gdf/B0505E.gdf and cause missing/mismatched eval labels.
    """
    if root_labels is None or not os.path.isdir(root_labels):
        return None

    subj = f"B{subject:02d}"
    phase = str(phase).upper()

    def _all_mat_files():
        hits = glob.glob(os.path.join(root_labels, "*.mat"))
        hits += glob.glob(os.path.join(root_labels, "*.MAT"))
        hits += glob.glob(os.path.join(root_labels, "**", "*.mat"), recursive=True)
        hits += glob.glob(os.path.join(root_labels, "**", "*.MAT"), recursive=True)
        return sorted(set(hits), key=lambda p: (len(os.path.basename(p)), p))

    def _basename_stem(path: str) -> str:
        return os.path.splitext(os.path.basename(path))[0].upper()

    # Session-specific lookup: only allow the exact requested session stem.
    # Example: subject=5, session=4, phase=E -> B0504E.mat only.
    if session is not None:
        exact_stem = f"{subj}{int(session):02d}{phase}".upper()

        direct_candidates = [
            os.path.join(root_labels, f"{exact_stem}.mat"),
            os.path.join(root_labels, f"{exact_stem}.MAT"),
        ]
        ...
        for path in _all_mat_files():
            if _basename_stem(path) == exact_stem:
                return path

        # Very conservative fuzzy fallback: still require the full session stem.
        # This supports names like true_B0504E_labels.mat but rejects B0501T.mat.
        fuzzy_hits = []
        for path in _all_mat_files():
            base = _basename_stem(path)
            if exact_stem in base:
                fuzzy_hits.append(path)

        return fuzzy_hits[0] if fuzzy_hits else None

    # Combined eval lookup: only accept aggregate subject eval files, never
    # per-session training labels and never individual B0504E/B0505E files.
    aggregate_stems = {
        f"{subj}{phase}",       # e.g. B05E
        f"{subj}_{phase}",      # e.g. B05_E
        f"{subj}_EVAL",
        f"{subj}EVAL",
        f"{subj}_TEST",
        f"{subj}TEST",
        subj,                   # e.g. B05, if it truly stores 320 eval labels
    }

    for path in _all_mat_files():
        base = _basename_stem(path)
        if base in aggregate_stems:
            return path

    # Conservative fuzzy aggregate fallback.
    # Reject B0501T/B0502T/B0503T and reject individual eval session files.
    fuzzy_hits = []
    for path in _all_mat_files():
        base = _basename_stem(path)

        if not base.startswith(subj):
            continue
        if base.endswith("T"):
            continue
        if base in {f"{subj}04E", f"{subj}05E"}:
            continue

        # Keep only aggregate-looking files.
        if any(token in base for token in ("EVAL", "TEST", "_E")) or base == subj or base == f"{subj}E":
            fuzzy_hits.append(path)

    return fuzzy_hits[0] if fuzzy_hits else None


def extract_subject_2b(subject: int, root_gdf: str, root_labels: str):
    """Load one BCI Competition IV-2b subject.

    Expected files:
      - B0101T.gdf, B0102T.gdf, B0103T.gdf for training/calibration
      - B0104E.gdf, B0105E.gdf for evaluation/test

    Labels returned are 1..2. The extracted window is 0..3.996 s relative to
    the cue/unknown event, corresponding to the usual 3..7 s MI window.
    """
    X_tr, y_tr = [], []
    X_te_parts, y_te_parts = [], []
    picked_names_last = None
    train_sessions = (1, 2, 3)
    eval_sessions = (4, 5)
    train_files, eval_files = [], []

    for session in train_sessions:
        gdf_path = _resolve_gdf_2b(root_gdf, subject, session, "T")
        train_files.append(os.path.basename(gdf_path))
        raw, events, event_id, picks, picked_names = _read_raw_and_events_2b(gdf_path)
        picked_names_last = picked_names
        _peek_events(os.path.basename(gdf_path), events, event_id)
        class_ids = _find_train_event_codes_2b(event_id)
        X_s, sel_s = _epoch_by_ids(raw, events, list(class_ids), picks)
        y_s = _derive_labels_from_event_codes_2b(sel_s, class_ids)
        if len(y_s) != X_s.shape[0]:
            raise ValueError(f"{gdf_path}: epochs={X_s.shape[0]} but labels={len(y_s)}")
        X_tr.append(X_s.astype(np.float32)); y_tr.append(y_s.astype(np.int64))

    missing_eval_label_sessions = []
    for session in eval_sessions:
        gdf_path = _resolve_gdf_2b(root_gdf, subject, session, "E")
        eval_files.append(os.path.basename(gdf_path))
        raw, events, event_id, picks, picked_names = _read_raw_and_events_2b(gdf_path)
        picked_names_last = picked_names
        _peek_events(os.path.basename(gdf_path), events, event_id)
        class_ids = _find_train_event_codes_2b(event_id)
        eval_id = _find_eval_event_code_2b(event_id)
        wanted_ids = [eval_id] if eval_id is not None else [i for i in class_ids if i is not None]
        X_s, sel_s = _epoch_by_ids(raw, events, wanted_ids, picks)

        mat_path = _resolve_label_mat_2b(root_labels, subject, session=session, phase="E")
        print(f"[LABEL {os.path.basename(gdf_path)}] file:", mat_path)
        y_s = _labels_from_mat_2a(mat_path, expect_n=X_s.shape[0])
        if y_s is None:
            try:
                y_s = _derive_labels_from_event_codes_2b(sel_s, class_ids)
        ...
        X_te_parts.append(X_s.astype(np.float32))
        y_te_parts.append(None if y_s is None else y_s.astype(np.int64))
        if y_s is None:
            missing_eval_label_sessions.append(session)

    if any(y is None for y in y_te_parts):
        total_eval_n = sum(x.shape[0] for x in X_te_parts)
        combined_mat = _resolve_label_mat_2b(root_labels, subject, session=None, phase="E")
        print(f"[LABEL B{subject:02d} combined E] file:", combined_mat)
        y_all = _labels_from_mat_2a(combined_mat, expect_n=total_eval_n)
        if y_all is None:
            raise ValueError(
                f"[IV-2b EVAL] missing/mismatched labels for subject B{subject:02d}; "
                f"missing sessions={missing_eval_label_sessions}, total_eval_epochs={total_eval_n}, "
                f"label_root={root_labels}"
            )
        y_te_parts = []
        st = 0
        for X_s in X_te_parts:
            ed = st + X_s.shape[0]
            y_te_parts.append(y_all[st:ed].astype(np.int64))
            st = ed

    X_tr = np.concatenate(X_tr).astype(np.float32)
    y_tr = np.concatenate(y_tr).astype(np.int64)
    X_te = np.concatenate(X_te_parts).astype(np.float32)
    y_te = np.concatenate(y_te_parts).astype(np.int64)

    print(f"[IV-2b] B{subject:02d} files train={train_files} eval={eval_files}")
    print(f"[IV-2b] B{subject:02d} picked channels: {picked_names_last}")
    print(f"[IV-2b] B{subject:02d} y_train counts: {dict(zip(*np.unique(y_tr, return_counts=True)))}")
    print(f"[IV-2b] B{subject:02d} y_test counts: {dict(zip(*np.unique(y_te, return_counts=True)))}")
    return X_tr, y_tr, X_te, y_te
...
def bandse_stats(branch, loader, use_abs_energy: bool = True):
    ...
    """
    if getattr(branch, "disable_bandse", False):
        return {
            "disabled": True,
            "mean": [], "std": [], "min": [], "max": [], "sparsity": 0.0,
        }
...
class _TemporalBranch(nn.Module):
    ...
    def __init__(self, f1: int, D: int, kernel_size: int, dil_temporal: int,
        ...
        self.ablation_mode = _norm_ablation_mode(ablation_mode)
        self.disable_sincnet = self.ablation_mode == "WO_SINCNET"
        self.disable_temporal_conv = self.ablation_mode == "WO_TEMPORAL_CONV_LONG_SHORT"
        self.disable_inception_tcn = self.ablation_mode == "WO_INCEPTION_TCN"
        self.disable_pointwise_fusion = self.ablation_mode == "WO_POINTWISE_CONV_FUSION"
        self.disable_bandse = self.ablation_mode == "WO_BANDSE"
        ...
        # Create BandSE only when it is part of the selected architecture.
        # WO_BANDSE has no BandSE/Identity placeholder and bypasses this stage directly.
        if not self.disable_bandse:
        ...
        self.itcn = nn.Identity() if self.disable_inception_tcn else _InceptionTCN(self.f2)
    ...
    def _bandse_weights_for_input(self, x: torch.Tensor, average: bool = True) -> torch.Tensor:
        """Return Conv/Sinc BandSE weights for diagnostics."""
        if self.disable_bandse:
            raise RuntimeError("BandSE diagnostics requested while WO_BANDSE is active.")
    ...
    def _spatial_features_pre_fuse(self, x: torch.Tensor):
        ...
        if self.disable_temporal_conv and not self.disable_sincnet:
            y_sinc = self.fb(x)
            y_conv = torch.zeros_like(y_sinc)
        elif self.disable_sincnet and not self.disable_temporal_conv:
            y_conv = self.conv_t1(x)
        ...
        if self.disable_bandse:
            y_conv = self.bn1_conv(y_conv)
            y_sinc = self.bn1_sinc(y_sinc)
        else:
        ...
        # Hard-zero after spatial path too, so removed paths cannot re-enter through
        # BatchNorm affine constants or learned downstream weights.
        if self.disable_temporal_conv:
            z_conv = torch.zeros_like(z_sinc)
        if self.disable_sincnet:
            z_sinc = torch.zeros_like(z_conv)
    ...
    def _front(self, x: torch.Tensor) -> torch.Tensor:
        z_conv, z_sinc = self._spatial_features_pre_fuse(x)
        if self.disable_pointwise_fusion:
            # No learnable 1x1 pointwise Conv fusion. Keep output channel count f2.
            if self.disable_temporal_conv and not self.disable_sincnet:
                return z_sinc
            if self.disable_sincnet and not self.disable_temporal_conv:
                return z_conv
            return 0.5 * (z_conv + z_sinc)
        return self.spatial_pointwise_fuse(torch.cat([z_conv, z_sinc], dim=1))
...
class DualScalePatchEmbeddingCNN(nn.Module):
    ...
    def __init__(self, f1_total: int = 8, D: int = 2, kernel_short: int = 32, kernel_long: int = 96,
        ...
        super().__init__()
        self.ablation_mode = _norm_ablation_mode(ablation_mode)
        self.disable_branch_i = self.ablation_mode == "WO_BRANCH_I"   # Branch I = Mu / long branch
        self.disable_branch_ii = self.ablation_mode == "WO_BRANCH_II" # Branch II = Beta / short branch
        ...
        self.branch_mu = _TemporalBranch(f1_branch, D, kernel_long, dil_long,
                                         ...
                                         init_bands=mu_init_bands, ablation_mode=self.ablation_mode)
        self.branch_beta = _TemporalBranch(f1_branch, D, kernel_short, dil_short,
                                           pooling_size1, pooling_size2, dropout_rate,
                                           number_channel, sample_rate, fixed_range=BETA_BAND,
                                           init_bands=beta_init_bands, ablation_mode=self.ablation_mode)
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.disable_branch_i:
            xb = self.branch_beta(x)
            return torch.cat([torch.zeros_like(xb), xb], dim=-1)
        if self.disable_branch_ii:
            xa = self.branch_mu(x)
            return torch.cat([xa, torch.zeros_like(xa)], dim=-1)
        xa = self.branch_mu(x)
        xb = self.branch_beta(x)
...
class EEGTransformer(nn.Module):
    def __init__(self, heads=2, emb_size=16, depth=10, database_type='B',
        ...
        self.ablation_mode = _norm_ablation_mode(ablation_mode)
        self.disable_transformer_encoder = self.ablation_mode == "WO_TRANSFORMER_ENCODER"
        ...
        self.trans = nn.Identity() if self.disable_transformer_encoder else TransformerEncoder(heads, depth, emb_size, causal_attention=causal_attention)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_transformer_encoder:
            feats = x_tok
        else:
            z = self.trans(x_tok)
            feats = x_tok + z
...
def build_adam_optimizer(model: nn.Module, lr: float, betas=(0.9, 0.999),
    ...
    """Build AdamW optimizer with safe weight-decay groups.

    This keeps the strong old IV-2a backbone structure while adapting optimizer/regularization for IV-2b. Weight decay is applied only to
    ...
    if weight_decay <= 0.0:
        print(f"[Optimizer] AdamW lr={lr:g}, betas={betas}, weight_decay=0", flush=True)
        return torch.optim.AdamW(model.parameters(), lr=lr, betas=betas)

    if not safe_groups:
        print(
            f"[Optimizer] AdamW lr={lr:g}, betas={betas}, weight_decay={weight_decay:g}, safe_groups=False",
            flush=True,
        )
    ...
    print(
        f"[Optimizer] AdamW lr={lr:g}, betas={betas}, weight_decay={weight_decay:g}, "
...
class ExP:
    ...
    def train(self):
        ...
        print(
            ...
            f"bands={self.bandmix_bands} source=official_training_class_pool",
            flush=True,
        )
            ...
            if is_best:
                ...
                with torch.no_grad():
                    ...
                    if self.ablation_mode == "WO_BANDSE":
                        w_mu = w_mu_conv = w_mu_sinc = None
                        w_beta = w_beta_conv = w_beta_sinc = None
                    else:
                        w_mu, w_mu_conv, w_mu_sinc = _bandse_quick_snapshot(
                            self.model.cnn.branch_mu, xb_diag
                        )
                        w_beta, w_beta_conv, w_beta_sinc = _bandse_quick_snapshot(
                            self.model.cnn.branch_beta, xb_diag
                        )
                    ...
                    print(f"[Best@{e}] val_acc={vacc:.6f} val_loss={vloss.item():.7f}")
                    if self.ablation_mode == "WO_BANDSE":
                        print(f"[Best@{e}] BandSE: DISABLED (WO_BANDSE)")
                    else:
...
def run_with_arrays(X_train, y_train, X_test, y_test, **cfg):
    ...
    exp = ExP(
        ...
        dataset_type=cfg.get('DATASET_TYPE', DATASET_TYPE), f1_total=cfg['EEG_F1_TOTAL'], D=cfg['EEG_D'],
        ...
        use_aug=(int(cfg.get('N_AUG', 0)) > 0 and cfg.get('AUG_MODE', AUG_MODE) != 'NO_AUG'),
...
# ============== Main logic ==============
def main():
    print(f"[IV-2b] Loading Subject B{SUBJECT:02d} from: {ROOT_GDF}, labels: {ROOT_LABELS}")
    X_tr, y_tr, X_te, y_te = extract_subject_2b(SUBJECT, ROOT_GDF, ROOT_LABELS)
    print(f"[Subject {SUBJECT}] Train: {X_tr.shape}, Test: {X_te.shape}")
    acc, _, _, _ = run_with_arrays(
        X_tr, y_tr, X_te, y_te,
        SUBJECT=SUBJECT,
        result_dir=os.path.join(
            OUTPUT_ROOT, ABLATION_MODE.lower(), f"subject_{SUBJECT}", f"seed_{SEED}"
        ),
        ...
        LR=LR, BATCH_SIZE=BATCH_SIZE, SEED=SEED, DATASET_TYPE=DATASET_TYPE,
        ABLATION_MODE=ABLATION_MODE
    )
    print("=" * 60)
    print(f"[Subject {SUBJECT}] Final Accuracy (IV-2b): {acc:.4f}")
    print("=" * 60)


if __name__ == "__main__":
    """
    Run IV-2b for all configured ablation modes, Subjects 1..9,
    and fixed seeds 0,1,2,3,4.

    Output:
      - One folder per ablation, subject and seed under OUTPUT_ROOT.
      - all_runs.csv, per_subject_5seed_summary.csv and overall_summary.csv.
    """
    print(f"[IV-2b] Attention mode: {'causal-masked self-attention' if CAUSAL_ATTENTION else 'full self-attention / no causal mask'}")
    print(f"[IV-2b] Run subjects={SUBJECTS} with fixed seeds={FIXED_SEEDS}")
    print(f"[IV-2b] Augmentation mode={AUG_MODE}")
    print(f"[IV-2b] Ablation modes={ABLATION_MODES}")
    print(f"[IV-2b] GDF root: {ROOT_GDF}")
    print(f"[IV-2b] Label root: {ROOT_LABELS}")
    os.makedirs(OUTPUT_ROOT, exist_ok=True)
    print(f"[IV-2b] Output root: {OUTPUT_ROOT}")
    ...
    base_cfg_dump = {
        ...
        "BEST_BY": BEST_BY,
        "OPTIMIZER": "AdamW_weight_decay_safe_groups" if WEIGHT_DECAY_SAFE_GROUPS else "AdamW_weight_decay_all_params",
        "EA_APPLY": EA_APPLY,
        ...
        "TEMPORAL_KERNEL_VARIANT": "KSHORT64_KLONG96_DSHORT1_DLONG3",
    ...
    for subject in SUBJECTS:
        print("\n" + "=" * 90)
        print(f"[IV-2b] Loading Subject {subject} once, then running 5 fixed seeds")
        print("=" * 90)
        ...
        for ablation_mode in ABLATION_MODES:
            print("\n" + "#" * 90)
            print(f"[IV-2b] Subject {subject} | ABLATION={ablation_mode}")
            print("#" * 90)

            for seed in FIXED_SEEDS:
                print("\n" + "-" * 90)
                print(f"[IV-2b] Subject {subject} | ablation={ablation_mode} | seed={seed}")
                print("-" * 90)

                # Reset seed before each run so model init, shuffle, augmentation,
                # and validation split are controlled by the requested seed.
                ...
                print(
                    f"[Subject {subject} | ablation={ablation_mode} | seed={seed}] Final Accuracy: "
                    f"{acc:.4f} | kappa={kap:.4f} | best_epoch={best_ep} | time={dur/60:.1f}m"
                )
        ...
        print("\n" + "*" * 90)
        print(f"[Subject {subject}] per-ablation seed summary:")
        for _abl, _df_abl in df_sub_tmp.groupby("ablation"):
            print(
                f"  {_abl}: acc={_df_abl['acc'].mean():.4f} ± {_df_abl['acc'].std(ddof=1):.4f} | "
                f"kappa={_df_abl['kappa'].mean():.4f} ± {_df_abl['kappa'].std(ddof=1):.4f}"
            )
        print("*" * 90)
    ...
    # Save all individual runs: len(ABLATION_MODES) x 9 subjects x 5 seeds.
    df_all = pd.DataFrame(all_rows)
    csv_all = os.path.join(OUTPUT_ROOT, "all_runs.csv")
    df_all.to_csv(csv_all, index=False)

    # Per-subject mean ± std over 5 seeds
    df_subject = (
        df_all
        .groupby(["ablation", "subject"], as_index=False)
        .agg(
    ...
    csv_subject = os.path.join(OUTPUT_ROOT, "per_subject_5seed_summary.csv")
    df_subject.to_csv(csv_subject, index=False)

    # Overall summaries per ablation:
    # 1) Mean over subject-level 5-seed means: preferred final IV-2b score.
    # 2) Mean over all subject-seed runs: useful sanity check.
    overall_rows = []
    for ablation_mode, df_subject_abl in df_subject.groupby("ablation"):
        df_all_abl = df_all[df_all["ablation"] == ablation_mode]
        overall_rows.append(
            {
                "ablation": ablation_mode,
                "summary_type": "mean_over_subject_5seed_means",
                "n_subjects": int(df_subject_abl.shape[0]),
                "n_runs": int(df_all_abl.shape[0]),
                "acc_mean": float(df_subject_abl["acc_mean_5seed"].mean()),
                "acc_std": float(df_subject_abl["acc_mean_5seed"].std(ddof=1)),
                "kappa_mean": float(df_subject_abl["kappa_mean_5seed"].mean()),
                "kappa_std": float(df_subject_abl["kappa_mean_5seed"].std(ddof=1)),
            }
        )
        overall_rows.append(
            {
                "ablation": ablation_mode,
                "summary_type": "mean_over_all_subject_seed_runs",
                "n_subjects": int(df_subject_abl.shape[0]),
                "n_runs": int(df_all_abl.shape[0]),
                "acc_mean": float(df_all_abl["acc"].mean()),
                "acc_std": float(df_all_abl["acc"].std(ddof=1)),
                "kappa_mean": float(df_all_abl["kappa"].mean()),
                "kappa_std": float(df_all_abl["kappa"].std(ddof=1)),
            }
        )
    df_overall = pd.DataFrame(overall_rows)
    csv_overall = os.path.join(OUTPUT_ROOT, "overall_summary.csv")
    df_overall.to_csv(csv_overall, index=False)

    print("\n" + "=" * 90)
    print("[IV-2b] ALL SUBJECTS × 5 SEEDS × ARCHITECTURE ABLATIONS DONE")
    print("\n[All individual runs]")
    print(df_all.to_string(index=False))
    print("\n[Per-subject 5-seed summary]")
    print(df_subject.to_string(index=False))
    print("\n[Overall summary]")
    print(df_overall.to_string(index=False))
    print("-" * 90)
    print(f"Saved individual runs to: {csv_all}")
    print(f"Saved per-subject summary to: {csv_subject}")
    print(f"Saved overall summary to: {csv_overall}")
    print(f"Total wall time: {(time.time() - t0_all) / 60:.1f}m")
    print("=" * 90)
